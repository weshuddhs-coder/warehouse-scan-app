...
export default function ScanPickedUp() {
  ...
  const loadCounts = async () => {
    try {
      ...
      if (data) setTodayCount(data.picked_up_today || 0)
    } catch (err) {
  ...
  const loadRecentScans = async () => {
    try {
      const { data, error } = await supabase.functions.invoke('warehouse_recent_scans', {
        body: { operator, screen: 'PICKED_UP', limit: 5 }
      })
      if (error) throw error
  ...
  const handleScan = async (awb) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('warehouse_scan_picked_up', {
        body: { awb, operator }
      })
      ...
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Scan PICKED UP</h1>
            <p className="text-gray-600 mt-2">Mark parcels handed to courier</p>
          </div>
          ...
          <input
            ...
            className="w-full p-4 border-2 border-gray-300 rounded-lg mb-4 text-lg focus:border-orange-500 focus:outline-none"
            autoFocus
          />
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-orange-600 text-white p-4 sticky top-0 shadow-lg z-10">
        <div className="max-w-md mx-auto">
          <div className="flex justify-between items-center mb-3">
            <h1 className="text-2xl font-bold">🚚 PICKED UP Scan</h1>
            <button
              onClick={toggleSound}
          ...
          <div className="flex gap-4">
            <div className="flex-1 bg-orange-500 p-3 rounded-lg">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg p-4">
        <div className="max-w-md mx-auto flex gap-3">
          ...
          <button
            onClick={() => navigate('/scan-ready')}
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-lg font-semibold transition-colors"
          >
            ← Switch to READY
          </button>
        </div>
